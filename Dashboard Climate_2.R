...
server <- function(input, output,session){
  ...
  output$scatterplot <- renderPlot({
    ...
    for (v in input$vars_korelasi) {
      if (v == input$dep_var) {          # variabel yang sama dengan Y terpilih
  ...
  # ===================================================================
  # OUTPUT UNTUK TAB METADATA (VERSI TABEL)
  # ===================================================================
  
  output$tampil_metadata_variabel <- renderUI({
    # Pastikan input sudah dipilih
    req(input$pilih_variabel_metadata)
    
    # Ambil informasi metadata dari kamus data berdasarkan pilihan input
    info <- metadata_kamus[[input$pilih_variabel_metadata]]
    
    # Ubah list menjadi data frame untuk ditampilkan sebagai tabel
    df_info <- data.frame(
      Atribut = names(info),
      Keterangan = unlist(info),
      row.names = NULL
    )
    
    # Buat tabel HTML secara manual agar lebih fleksibel
    tags$table(class = "table table-striped table-bordered",
               style = "width: 100%;",
               # Header Tabel
               tags$thead(
                 tags$tr(
                   tags$th("Atribut", style = "width: 25%;"),
                   tags$th("Keterangan")
                 )
               ),
               # Isi Tabel
               tags$tbody(
                 # Gunakan lapply untuk membuat baris tabel secara dinamis
                 lapply(1:nrow(df_info), function(i) {
                   tags$tr(
                     tags$td(tags$strong(df_info$Atribut[i])),
                     tags$td(df_info$Keterangan[i])
                   )
                 })
               )
    )
  ...
  data_stat_bencana <- reactive({
    # Pastikan semua input yang dibutuhkan sudah tersedia
    req(input$bencana_terpilih_stat, input$rentang_tahun_bencana_stat)
    ...
    rentang_tahun <- input$rentang_tahun_bencana_stat
    
    data_dashboard %>%
      # --- TAMBAHKAN FILTER INI ---
      # Filter data berdasarkan rentang tahun yang dipilih
      filter(tahun >= rentang_tahun[1] & tahun <= rentang_tahun[2]) %>%
      # ---------------------------
    select(provinsi, tahun, all_of(kolom_kejadian), all_of(kolom_korban)) %>%
      rename(
        `Jumlah Kejadian` = all_of(kolom_kejadian),
        `Jumlah Korban Jiwa` = all_of(kolom_korban)
      ) %>%
      arrange(desc(tahun), provinsi)
  })
      ...
      return(
        div(
          ...
          tagList(
            ...
            h4("Metodologi", style = "font-weight: bold;"),
            p("Analisis menggunakan metode statistik deskripti, regresi linear, dan visualisasi spasial untuk mengidentifikasi pola dan tren data iklim")
          )
        )
      )
    } else {
